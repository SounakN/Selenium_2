...
public class ActionMethods {
	...
	public void sync1(WebDriver driver, WebElement element) {
		try {
			Wait<WebDriver> wait = new WebDriverWait(driver, 100).ignoring(NoSuchElementException.class);
			wait.until(ExpectedConditions.visibilityOf(element));
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void highlightElement(WebDriver driver, WebElement element) {
		try {
			JavascriptExecutor js = (JavascriptExecutor) driver;
			js.executeScript("arguments[0].setAttribute('style', 'background: LightGreen; border: 2px solid yellow;' )",
					element);
			Thread.sleep(1000);
	...
	public void checkcontainsTextForParticularElement(WebDriver driver, WebElement element, String str) {
		...
		try {
			/*
			 * String containsObject = "//*[contains(text(),'" + str + "')]"; new
			 * WebDriverWait(driver,30).until(ExpectedConditions.
			 * visibilityOfAllElementsLocatedBy(By.xpath(containsObject))); return true;
			 */
	...
	/*
	 * public String DownloadFile(WebDriver driver,WebElement element) throws
	 * Throwable {
	 * 
	 * String attachmentDocPath = null; // List the Download location files before
	 * Export to Excel ArrayList<String> filesInDirectoryBefore =
	 * (ArrayList<String>) getDirectoryInfo();
	 * System.out.println(filesInDirectoryBefore.size());
	 * 
	 * // Perform Export to Excel and Save in default Downloads location
	 * click(element); Thread.sleep(4000);
	 * 
	 ...
	 * Thread.sleep(1000); robot.keyPress(KeyEvent.VK_S);
	 * robot.keyRelease(KeyEvent.VK_ALT); robot.keyRelease(KeyEvent.VK_S);
	 * 
	 * System.out.println("Robot is entered"); Actions action = new Actions(driver);
	 * action.sendKeys(Keys.chord(Keys.ALT, "S")).build().perform();
	 * action.sendKeys(Keys.ENTER).build().perform();
	 * 
	 * Thread.sleep(5000);
	 * 
	 * // List the Download location files after Export to Excel ArrayList<String>
	 * filesInDirectoryAfter = (ArrayList<String>) getDirectoryInfo();
	 * System.out.println(filesInDirectoryAfter.size()); // Validate whether any new
	 * download has taken place Assert.assertNotEquals(filesInDirectoryBefore,
	 * filesInDirectoryAfter);
	 * 
	 * // Differentiate and list only the new files
	 * filesInDirectoryAfter.removeAll(filesInDirectoryBefore);
	 * 
	 * //Log.info("New File downloaded on Export to Excel: " +
	 * filesInDirectoryAfter);
	 * 
	 * // Copy to files to ./Doc file under project directory for future //
	 * reference. for (int newFileCtr = 0; newFileCtr <
	 * filesInDirectoryAfter.size(); newFileCtr++) {
	 * 
	 * File sourceFile = new File( System.getProperty("user.home") + "/Downloads/" +
	 * filesInDirectoryAfter.get(newFileCtr)); if (!new File("./Docs").exists())
	 * (new File("./Docs")).mkdir();
	 * 
	 ...
	 * 
	 * 
	 * FileUtils.copyFile(sourceFile, destFile); attachmentDocPath =
	 ...
	 * 
	 * private Collection<String> getDirectoryInfo() { List<String> fileNameList =
	 * new ArrayList<String>(); File file = new File(System.getProperty("user.home")
	 * + "/Downloads"); if (file.isDirectory()) { String[] fileList = file.list();
	 * fileNameList.addAll(Arrays.asList(fileList)); } return fileNameList;
	 }*/
	...
	public static Connection executeSQLQuery(String connectionName, String portNo, String SID, String uNAme,
			String pswd, String Query) {
		Connection con = null;
		try {
	 ...
	 * SetUp.driver.manage().timeouts().implicitlyWait(0,TimeUnit.SECONDS); String
	 * s=""; try { if(SetUp.driver.findElements(By.xpath(x)).size() != 0)
	 * s=SetUp.driver.findElement(By.xpath(x)).getText();
	 * 
	 * else return "0,0"; }catch(Exception e) { return "0,0"; } return s; }
	 */
